...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
		...
		try {
			if (!txObject.hasConnectionHolder() ||
				...
				if (logger.isDebugEnabled()) {
					logger.info("Acquired Connection [" + newCon + "] for JDBC transaction");
				}
			...
			if (con.getAutoCommit()) {
				...
				if (logger.isDebugEnabled()) {
					logger.info("Switching JDBC Connection [" + con + "] to manual commit");
				}
	...
	protected void doCommit(DefaultTransactionStatus status) {
		...
		if (status.isDebug()) {
			logger.info("Committing JDBC transaction on Connection [" + con + "]");
		}
		try {
			con.commit();
		}
	...
	protected void doRollback(DefaultTransactionStatus status) {
		...
		Connection con = txObject.getConnectionHolder().getConnection();
		if (status.isDebug()) {
			logger.info("Rolling back JDBC transaction on Connection [" + con + "]");
		}
		try {
			con.rollback();
		}
	...
	protected void doSetRollbackOnly(DefaultTransactionStatus status) {
		DataSourceTransactionObject txObject = (DataSourceTransactionObject) status.getTransaction();
		if (status.isDebug()) {
			logger.info("Setting JDBC transaction [" + txObject.getConnectionHolder().getConnection() +
					"] rollback-only");
		}
