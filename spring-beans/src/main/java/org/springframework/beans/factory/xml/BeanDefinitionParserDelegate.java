...
//			如果不存在beanname，spring根据命名规则为当前bean生成名字
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			//根据<Bean>元素配置的class名称和parent属性值创建GenericBeanDefinition
			//用于承接属性的
			AbstractBeanDefinition bd = createBeanDefinition(className, parent);

			//硬编码对当前的<Bean>元素中配置的一些属性进行解析和设置，如配置的单态(singleton)属性等
			parseBeanDefinitionAttributes(ele, beanName, containingBean, bd);
			//为<Bean>元素解析的Bean设置description信息
			bd.setDescription(DomUtils.getChildElementValueByTagName(ele, DESCRIPTION_ELEMENT));
			//对<Bean>元素的meta(元信息)属性解析
//			<bean id="beanName"><meta key="" value=""></bean>    这个配置不会体现在beanName中，只是服务于BeanDefinition获取
			parseMetaElements(ele, bd);
			//对<Bean>元素的lookup-method属性解析
			// 获取器注入，是一种特殊的方法注入，对方法返回的bean可在配置文件里指定，来代替方法返回的bean。用于可插拔时的设计
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
			//对<Bean>元素的replaced-method属性解析
			//方法替换，运行时用新的方法替换现有的方法
