...
abstract class AbstractRecursiveAnnotationVisitor extends AnnotationVisitor {
	...
	protected Object getEnumValue(String asmTypeDescriptor, String attributeValue) {
		...
		try {
			...
			logger.info("Failed to classload enum type while reading annotation metadata", ex);
		}
		catch (IllegalAccessException | AccessControlException ex) {
			logger.info("Could not access enum value while reading annotation metadata", ex);
		}
		return valueToUse;
	}
