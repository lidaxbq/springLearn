...
public class JmsTransactionManager extends AbstractPlatformTransactionManager
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.info("Created JMS transaction on Session [" + session + "] from Connection [" + con + "]");
			}
	...
	protected void doCommit(DefaultTransactionStatus status) {
		...
		if (session != null) {
			try {
				if (status.isDebug()) {
					logger.info("Committing JMS transaction on Session [" + session + "]");
				}
				session.commit();
			}
	...
	protected void doRollback(DefaultTransactionStatus status) {
		...
		if (session != null) {
			try {
				if (status.isDebug()) {
					logger.info("Rolling back JMS transaction on Session [" + session + "]");
				}
				session.rollback();
			}
			catch (JMSException ex) {
