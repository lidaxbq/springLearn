	...
	protected Class<?> determineActivationSpecClass(ResourceAdapter adapter) {
		...
		if (adapterClassName.endsWith(RESOURCE_ADAPTER_SUFFIX)) {
			...
			try {
				...
				logger.info("No default <Provider>ActivationSpec class found: " + specClassName);
			}
		}
			...
			try {
				...
				logger.info("No default <Provider>ActivationSpecImpl class found: " + specClassName);
			}
		}
		...
		try {
			...
			logger.info("No default ActivationSpecImpl class found in provider package: " + specClassName);
		}
		...
		try {
			...
			logger.info("No default ActivationSpecImpl class found in inbound subpackage: " + specClassName);
		}
